...
import type { ProductionAIResult } from "@/lib/ai/production-ai";
...
export default function ProductionAIWidget({ result }: { result: ProductionAIResult }) {
  return (
    <section className="rounded-[28px] border border-cyan-500/30 bg-slate-950/80 p-5 shadow-2xl shadow-cyan-950/20">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Kpi label="Órdenes" value={result.totalOrders} />
        <Kpi label="Pendientes" value={result.pendingOrders} />
        <Kpi label="Proceso" value={result.processOrders} />
        <Kpi label="Atrasadas" value={result.delayedOrders} danger={result.delayedOrders > 0} />
      </div>

      <div className="mt-4 space-y-2">
        {result.alerts.slice(0, 6).map((alert, index) => (
      ...
      {result.recommendations.length > 0 && (
        <div className="mt-4 rounded-2xl border border-purple-400/20 bg-purple-400/10 p-4">
          <p className="mb-2 flex items-center gap-2 text-sm font-black text-purple-200">
            <Lightbulb size={16} /> Prioridades IA
          </p>
...
function Kpi({ label, value, danger = false }: { label: string; value: number; danger?: boolean }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-3">
      ...
      <p className={`mt-1 text-lg font-black ${danger ? "text-red-300" : "text-white"}`}>{value}</p>
    </div>
  );
}
