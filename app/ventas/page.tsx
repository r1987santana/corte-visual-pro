...
export default function VentasProClient() {
    ...
    try {
      ...
      const { data: sale, error: saleError } = await supabase
        ...
        .insert({
          ...
          workflow_status: "factura_pendiente_caja_principal",
          amount_paid: 0,
          balance: total,
        })
        .select("*")
      ...
      await writeAuditLog({
        module: "ventas",
        action: "invoice_created_pending_cashier",
        entity_type: "sales",
        entity_id: sale.id,
        entity_name: sale.invoice_number || invoiceNumber,
        new_data: {
          sale,
          items: itemsPayload,
      ...
      alert(`Factura pendiente creada: ${sale.invoice_number || invoiceNumber}. El cobro debe hacerse en Caja Principal.`);
  ...
  async function printInvoice(sale: SaleRecord, items: SaleItemRecord[]) {
    ...
    // Busca los códigos reales del inventario aunque la venta vieja no tenga code guardado.
    ...
    let inventoryCodeMap = new Map<string, string>();
    ...
    if (productIds.length > 0) {
      ...
      if (!invError && invRows) {
        inventoryCodeMap = new Map(
          invRows.map((row: any) => [String(row.id), String(row.code || "-")])
        );
      }
    }
    ...
    autoTable(doc, {
      ...
      body: items.map((item: any) => {
        const realCode =
          item.code ||
          item.product_code ||
          inventoryCodeMap.get(String(item.product_id || "")) ||
          "-";
      ...
      <section className="grid gap-5 p-6 lg:grid-cols-[1.35fr_1fr] lg:p-8">
        ...
        <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-5 shadow-2xl shadow-black/30">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400">
                Venta directa por unidad. El dinero solo se registra en Caja Principal.
              </p>
            </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <Field label="Cobro">
              <div className="input-pro flex items-center gap-2 text-emerald-200">
                <CreditCard size={16} />
                Caja Principal
              </div>
            </Field>
          ...
          >
            ...
            Crear factura pendiente para Caja Principal
          </button>
        </div>
      </section>
