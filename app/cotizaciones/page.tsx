...
function BoardOptimizerPreview({
  result,
  boardWidth,
  boardHeight,
  grainDirection,
    ...
    >
      ...
      {result.oversized.length > 0 && (
        <div className="mt-4 rounded-2xl border border-red-500/30 bg-red-500/10 p-3 text-xs font-bold text-red-200">
          Hay {result.oversized.length} pieza(s) fuera de medida para la plancha seleccionada. Revisa largo/ancho, veta o tamaño de tablero.
        </div>
      )}
        ...
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[11px] font-bold text-slate-500">
              Veta: {grainDirection === "sin_veta" ? "sin restricción" : grainDirection}. Preview aproximado para cotizar y validar.
            </p>
          </div>
...
function ServiceConfiguration(props: {
  materialOwner: MaterialOwner;
  setMaterialOwner: (v: MaterialOwner) => void;
  boardType: string;
  setBoardType: (v: string) => void;
  boardSize: BoardSize;
  setBoardSize: (v: BoardSize) => void;
  boardWidthMm: string;
  setBoardWidthMm: (v: string) => void;
  boardHeightMm: string;
  setBoardHeightMm: (v: string) => void;
  thicknessMm: string;
  setThicknessMm: (v: string) => void;
  boardColor: string;
  setBoardColor: (v: string) => void;
  grainDirection: GrainDirection;
  setGrainDirection: (v: GrainDirection) => void;
  cutPrice: string;
  setCutPrice: (v: string) => void;
  edgePrice: string;
  setEdgePrice: (v: string) => void;
  cncPrice: string;
  setCncPrice: (v: string) => void;
  serviceDescription: string;
  setServiceDescription: (v: string) => void;
}) {
  return (
    <PanelDark
      title="Configuración técnica del servicio"
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              props.setMaterialOwner(e.target.value as MaterialOwner)
            }
            className="input-dark"
          >
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => props.setBoardSize(e.target.value as BoardSize)}
            className="input-dark"
          >
        ...
        <InputDark
          label="Espesor mm"
          value={props.thicknessMm}
          setValue={props.setThicknessMm}
          type="number"
          placeholder="18"
        />
        <InputDark
          label="Ancho tablero mm"
          value={props.boardWidthMm}
          setValue={props.setBoardWidthMm}
          type="number"
          placeholder="1220"
        />
        <InputDark
          label="Alto tablero mm"
          value={props.boardHeightMm}
          setValue={props.setBoardHeightMm}
          type="number"
          placeholder="2440"
        />
        <InputDark
          ...
          setValue={props.setBoardColor}
          placeholder="Blanco, Bardolino, Wengué..."
        />
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              props.setGrainDirection(e.target.value as GrainDirection)
            }
            className="input-dark"
          >
        ...
        <InputDark
          label="Precio corte / pie"
          value={props.cutPrice}
          setValue={props.setCutPrice}
          type="number"
          placeholder="30"
        />
        <InputDark
          label="Precio canto / metro"
          value={props.edgePrice}
          setValue={props.setEdgePrice}
          type="number"
          placeholder="35"
        />
        <InputDark
          label="Precio CNC / unidad"
          value={props.cncPrice}
          setValue={props.setCncPrice}
          type="number"
          placeholder="0"
        />
        <div className="md:col-span-2">
...
function ThDark({ children }: { children: React.ReactNode }) {
  return (
...
function TotalRow({
  ...
  return (
    <div className="flex items-center justify-between py-1">
      <span
        className={`${big ? "text-lg" : "text-sm"} font-bold text-slate-300`}
      >
        {label}
      </span>
      <strong
...
function TypeBadge({ value }: { value: string }) {
  ...
  const label = v === "servicio" ? "Servicio" : "Artículo";
  return (
...
function QuoteAIControlPanel({
  ...
  const statusLabel = dangerCount > 0 ? "Riesgo alto" : warningCount > 0 ? "Revisar" : total > 0 ? "Saludable" : "Esperando datos";
  const statusClass =
    dangerCount > 0
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs font-semibold text-slate-400">
              Analiza margen, stock, errores, plancha y próximos pasos.
            </p>
          </div>
